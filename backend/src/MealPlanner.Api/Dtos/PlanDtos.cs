...
public sealed record ShoppingListRequest(Guid? PlanId, Guid[]? PlanIds, string[]? Dates, bool? OnlyMissing);

public sealed record MealPlanDto(Guid Id, Guid UserId, int Year, int Month, DateTimeOffset CreatedAtUtc, MealPlanDayDto[] Days);

public sealed record MealPlanDayDto(string Date, bool IsCompleted, PlannedMealDto[] Meals);
